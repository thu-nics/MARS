...
class LeducPoker(BaseDiscreteActionEnv):
    ...
    def _get_prefix_prompt(self, think=True, player_id=0):
        ...
        rules = (
            ...
            "    b. <CALL>: match the current bet. If no bet has been made, this is equivalent to checking.\n"
            "    c. <RAISE>: add n chips to the current bet, where n=2 in the first round and n=4 in the second round.\n"
            "6. A maximum of two <RAISE>s are allowed in each round. The round ends when both players have acted and their bets are equal.\n"
...
if __name__ == "__main__":
    # Basic unit test
    print("-" * 100)
    print("Leduc Poker Basic unit test:")
    print("-" * 100)
    env = LeducPoker(LeducPokerConfig(built_in_opponent="none"))
